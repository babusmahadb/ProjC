...
def vol_unmnt(cluster: str, vol_name: str, vol_uuid: str, headers_inc: str):
    ...
    if "root" in vol_name:
        ...
        if mnt == 'y':
            ...
            exit()        
def vol_rename(cluster: str,svm_name:str, vol_name: str, vol_uuid: str, chgnum: str, headers_inc: str):

    """ Rename Volume """
    new_name = vol_name
    dataobj = {}
    if "root" in vol_name:
        print("Volume "+bcolors.OKBLUE +vol_name+bcolors.ENDC +" is not recommended to r enamed, since it's root volume.")
        return new_name
    else:    
    ...
    return cluster,svm_name,new_name,vol_uuid,
...
if __name__ == "__main__":
    ...
    vol_data = "C:\\Users\\Administrator.DEMO\\Documents\\ProjC\\test\\Phase1_Validation.xlsx"
    ...
    print()
    vd_df = vols_df = vold_df = nc_df = qq_df = cc_df = pd.DataFrame([], columns=None, index=None)
    for ind in vol_df.index:
        ...
        dscls_name = vol_df['Target_Clus_IP'][ind]
       ...
       # print(bcolors.WARNING+ vol_name +bcolors.ENDC)
        ...
        if snpmir_check == 'Yes':
            #print()
            tgt_cls = vol_df['Target_Clus_IP'][ind]
        ...
        if snpmir_check == 'Yes':
            ...
            if dest == 'y':
                ...
                vname = vol_rename(cls_name,svm_name,vol_name,vol_uuid,chgnum,headers) #To_Excel -> vname,vol_uuid,cls_name
                
                print()
                print("Proceeding with Destination Volume"+bcolors.HEADER+dsvol_name+bcolors.HEADER+bcolors.ENDC)
                dname = vol_rename(dscls_name,svm_name,dsvol_name,dsvol_uuid,chgnum,headers) # To_Excel -> dname,dsvol_uuid,dscls_name
                               
                
                vols_df = pd.DataFrame(data=vname,columns=None,index=None)
                vd_df=vd_df.append(vols_df.T, ignore_index=True)
                vold_df = pd.DataFrame(data=dname,columns=None,index=None)
                vd_df = vd_df.append(vold_df.T, ignore_index=True)
                #print(vold_df)
                #vd_df=vd_df.append(vold_df, ignore_index=True)
                print(vd_df)
                #vd_df = pd.DataFrame(data=dname,columns=None,index=None)
                #vols_df = vols_df.append(vols_df, ignore_index=True)
                ...
                vname = vol_rename(cls_name,svm_name,vol_name,vol_uuid,chgnum,headers)
        else:
            print("No Snapmirror, Proceeding with Volume Rename....")
            vname = vol_rename(cls_name,svm_name,vol_name,vol_uuid,chgnum,headers)
            #print(vname)
            #svoff = cls_name + vol_name 
            #print(svoff)
            
            voldet_df = pd.DataFrame(data=vname,columns=None,index=None)
        ...
        if snpmir_check == 'Yes':
            ...
            if dest == 'y':
                ...
                vol_offline(cls_name,vname[1],vol_uuid,headers)
                print()
                print("Proceeding with Destination Volume")
                vol_offline(dscls_name,dname[1],dsvol_uuid,headers)
            ...
            vol_offline(cls_name,vname[1],vol_uuid,headers)
    writer = pd.ExcelWriter(r'C:\\Users\\Administrator.DEMO\\Documents\\ProjC\\test\\Phase1.xlsx')
    vd_df.to_excel(writer,sheet_name='VolDetails', index=False, header=['Cluster','svm','Volume_name','Volume_uuid'])
    writer.save()          
    print()
